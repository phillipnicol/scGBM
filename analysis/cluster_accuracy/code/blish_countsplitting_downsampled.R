...
meta <- readRDS("../../data/blish_meta.RDS")
...
Ysub <- Y[,sampled_indices]
colnames(Ysub) <- colnames(Y)[sampled_indices]
rownames(Ysub) <- rownames(Y)

print(dim(Y))

blish_countsplit <- function(Y1, Y2) {
  ...
  gbm.mono <- sum(choose(table(cluster1[sampled_indices], cluster2), 2)) / sum(choose(table(cluster2), 2))
  ...
  apr.mono <- sum(choose(table(cluster1[sampled_indices], cluster2), 2)) / sum(choose(table(cluster2), 2))
  ...
  lpca.mono <- sum(choose(table(cluster1[sampled_indices], cluster2), 2)) / sum(choose(table(cluster2), 2))
  ...
  sct.mono <- sum(choose(table(cluster1[sampled_indices], cluster2), 2)) / sum(choose(table(cluster2), 2))
  ...
  lpca_ns.mono <- sum(choose(table(cluster1[sampled_indices], cluster2), 2)) / sum(choose(table(cluster2), 2))
  ...
  glmpca.mono <- sum(choose(table(cluster1[sampled_indices], cluster2), 2)) / sum(choose(table(cluster2), 2))
...
for(i in 1:iters) {
  res[i, ] <- blish_countsplit(Y, Ysub)
  cat("RESULTS: ", res[i,], "\n")
}

saveRDS(res, file="../data/blish_countsplit_downsampled.RDS")
